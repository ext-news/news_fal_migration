 ...
 *
 */
class NewsFalMigrationCommandController extends CommandController
{
     ...
     * @param bool $start
     */
    public function runCommand($start = false)
    {


        $text = '';
        $status = $this->checkForUpdate($text);
        if (!$status) {
            $this->outputLine('no stat');
            $this->outputLine($text);
        } else {
            $this->outputLine('stat');
            $this->outputLine($text);
            if ($start) {
                $this->performUpdate();
            }
    ...
    public function checkForUpdate(&$description)
    {
        ...
        if ($notMigratedMediaRowsCount > 0) {
            $description = 'There are *' . $notMigratedMediaRowsCount . '* media records which are using the old media relation. '
        ...
        if ($notMigratedRelatedFileRowsCount > 0) {
            $description .= '<br /><br />There are *' . $notMigratedRelatedFileRowsCount . '* related file records which are using the old relation. '
                . 'This wizard will copy the files to "fileadmin/' . self::FOLDER_ContentUploads . '".';
            $updateNeeded = true;
        }
